...
def migrate(message):
    ...
    migrate_output = run_command(f'docker-compose exec -T user-service flask db migrate -m "{message}"')
    print(migrate_output)

    print("Migration successful. Running upgrade...")
    upgrade_output = run_command('docker-compose exec -T user-service flask db upgrade')
